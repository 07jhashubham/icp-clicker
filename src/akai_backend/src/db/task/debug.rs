...
fn get_all_tasks() -> Result<String, String>{
    ...
    let mut stmt = conn.prepare("SELECT id, completed_times, type, desc, data, classes, occupancy FROM Task").map_err(|e|e.to_string())?;
...
fn get_all_tasks_logs() -> Result<String, String>{
    let conn = CONN.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn.prepare("SELECT * FROM Task_logs").map_err(|e|e.to_string())?;
    let task_iter = stmt.query_map([], |row| {
        Ok(json!({
