...
use ic_cdk::{api::management_canister::{http_request::{CanisterHttpRequestArgument, HttpMethod, HttpResponse}, main::{create_canister, install_code, CanisterIdRecord, CanisterInstallMode, CreateCanisterArgument, InstallCodeArgument}}, call, println, spawn};
...
pub async fn poll_scale() {
    ...
    if available_size <= threshold {
        println!("Available stable memory: {} bytes, which is below the 20GB threshold.", available_size);
        
        scale().await.unwrap();
    } else {
        println!("Sufficient memory available: {} bytes. No scaling needed.", available_size);
    }
}
...
async fn register_child_canister(canister_id: String) -> Result<(), String>{
    call(Principal::from_text(env::var("MAIN_CANISTER").map_err(|e| e.to_string())?).map_err(|e| e.to_string())?, "add_child_canister", (canister_id,)).await.map_err(|e| format!("{:?}", e))?;
    Ok(())
}